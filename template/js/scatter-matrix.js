...
ScatterMatrix.prototype.render = function () {
  ...
  this.onData(function() {
    ...
    //??? popravi, da bo lepo
    var size_control = control.append('div').attr('class', 'form-group');
    var color_control = control.append('div').attr('class', 'form-group'); 
    var filter_control = control.append('div').attr('class', 'form-group');
    var variable_control = control.append('div').attr('class', 'form-group');
    var drill_control = control.append('div').attr('class', 'form-group');
    ...
    function set_filter(variable) {
      ...
      if (variable) {
        ...
        filter_control.append('label').attr('class', '"control-label col-md-5').append('p').text('Filter by ' + variable);
        var filter_li =
          filter_control
            .append('div')
            .attr('class', 'col-md-7')
    ...
    //        self.__draw(self.__cell_size, svg, color_variable, selected_colors, to_include, drill_variables);
    //      });
    if (to_include.length > 0) self.__cell_size *= 4/ to_include.length;
    contentWidth = $('#content').width();
    self.__cell_size *= 1.5 * contentWidth / 1650;
    self.__draw(self.__cell_size, svg, color_variable, selected_colors, to_include, drill_variables);

    //??? why is this text not centered?
    //color_control.append('label').attr('class', '"control-label col-md-8').append('p').text('Select a variable to color\n(click)');
    //color_control
    //  .append('div')
    //  .attr('class', 'col-md-2')
    //  .append('ul')
    ...
    color_variable = "Segment";
    selected_colors = undefined;
    self.__draw(self.__cell_size, svg, color_variable, selected_colors, to_include, drill_variables);

    variable_control.append('label').attr('class', 'control-label from-group col-md-2 ui-sortable').text('Include variables');
    var variable_li =
      variable_control
        .append('div')
        .attr('class', 'col-md-7')
        ...
        .enter().append('span').attr('style', 'padding-right:1.25em; display:inline-block;');
    ...
    variable_li.append('input')
               ...
               .on('click', function(d, i) {
                 ...
                 if (to_include.length > 0 && new_to_include.length) self.__cell_size *= to_include.length / new_to_include.length;
    ...
    //drill_control.append('label').attr('class', '"control-label col-md-5').append('p').text('Drill and Expand');
    //drill_li = 
    //  drill_control
    //    .append('div')
    //    .attr('class', 'col-md-7')
...
ScatterMatrix.prototype.__draw =
  ...
  this.onData(function() {
    ...
    //numeric_variables.sort().reverse();
    ...
    if (drill_variables.length > 0 && to_include.length > 0) size = size * to_include.length / drill_variables.length;
    ...
    // ??? why does brushing change position?
